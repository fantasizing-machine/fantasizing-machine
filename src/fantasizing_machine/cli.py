...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Fantasizing-Machine thinks for you about the meanings.",
        epilog="Example:  fantasizing-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | fantasizing-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | fantasizing-machine         #

    $ fantasizing-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="fantasizing-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"fantasizing-machine: {utterance}.")
        syslog.closelog()
